select
	TR.Nome as 'turma',
	AL.Aluno
From Turmas Tr Inner Join TurmaAlunos TA 
					on 
			TR.Id = TA.Turma inner join Alunos AL
							on 
					TA.Aluno = AL.id; --Traz alunos da turma
...
from Diarios D inner join TurmaAlunos TR
					on 
			D.Turma = TR.Turma
...
from Diarios D inner join TurmaAlunos TR 
					on 
			D.Turma = TR.Turma left join Presenca P
							on
		D.Id = P.Diario and TR.Aluno = P.Aluno left join Notas N 
									on 
						Tr.Aluno = N.Aluno and d.Id = N.Diario
